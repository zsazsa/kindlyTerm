...
fn tools() -> Vec<Tool> {
    ...
    vec![
        ...
        Tool { name: "send_text", description: "Type text into a terminal. Multi-line text is delivered as one bracketed paste when the program supports it (a shell then shows the block and waits for Enter instead of running each line). Set enter=true to press Enter afterwards. The user sees the terminal light up. Optional typing=<chars per second> types it out visibly over time instead of at once (the call still returns immediately; read the screen after text.len()/typing seconds).", schema: obj(json!({"terminal_id": tid, "text": {"type": "string"}, "enter": {"type": "boolean"}, "typing": {"type": "number", "description": "characters per second, e.g. 30; omit to send instantly"}}), &["terminal_id", "text"]) },
        Tool { name: "send_key", description: "Press a key: enter, tab, escape, backspace, delete, up/down/left/right, home, end, pageup, pagedown, f1..f12, or ctrl+<letter> (e.g. ctrl+c, ctrl+d, ctrl+z). Optional count>1 holds the key for that many auto-repeats (40ms apart; the call returns at once).", schema: obj(json!({"terminal_id": tid, "key": {"type": "string"}, "count": {"type": "integer", "description": "repeats, as if the key were held"}}), &["terminal_id", "key"]) },
        ...
        Tool { name: "set_viewport", description: "Set a canvas view: world x, y of the top-left and zoom. Returns the view and the canvas area size in screen pixels (area_w, area_h), so a caller can pan or zoom about a point in steps.", schema: obj(json!({"canvas_id": {"type": "integer"}, "x": {"type": "number"}, "y": {"type": "number"}, "zoom": {"type": "number"}, "animate": {"type": "boolean", "description": "glide there (default true) or jump"}}), &["canvas_id"]) },
        ...
        Tool { name: "create_group", description: "Frame items as a named group. arrange=true first packs them into a tidy grid (reading order kept) instead of framing them where they are.", schema: obj(json!({"item_ids": {"type": "array", "items": {"type": "integer"}}, "name": {"type": "string"}, "arrange": {"type": "boolean"}}), &["item_ids"]) },
        Tool { name: "arrange_group", description: "Pack a group's members into a tidy grid and shrink-wrap the frame around them. Returns the new frame.", schema: obj(json!({"group_id": {"type": "integer"}}), &["group_id"]) },
        Tool { name: "rename_group", description: "Rename a group.", schema: obj(json!({"group_id": {"type": "integer"}, "name": {"type": "string"}}), &["group_id", "name"]) },
        Tool { name: "add_to_group", description: "Grow a group's frame to take in an item (groups are frames: whatever sits inside is a member). Returns the members.", schema: obj(json!({"group_id": {"type": "integer"}, "item_id": iid}), &["group_id", "item_id"]) },
        Tool { name: "delete_group", description: "Dissolve a group (items stay).", schema: obj(json!({"group_id": {"type": "integer"}}), &["group_id"]) },
        Tool { name: "pin_item", description: "Pin an item to the screen (true) or release it (false). A pinned item is drawn at 1:1 on top of the canvas and ignores pan and zoom; optional x, y place it, in pixels from the canvas area's top-left (it stays on screen). move_item on a pinned item releases it back onto the canvas.", schema: obj(json!({"item_id": iid, "pinned": {"type": "boolean"}, "x": {"type": "number"}, "y": {"type": "number"}}), &["item_id", "pinned"]) },
        ...
        Tool { name: "dock_terminal", description: "Show a terminal in the top-right corner of every screen of its window, on top of whatever tab is active (on=false stops). Keeps the terminal the user talks to in view while they work elsewhere.", schema: obj(json!({"terminal_id": tid, "on": {"type": "boolean"}}), &["terminal_id"]) },
